...
public class GraphicRenderer {
	...
	public void display(){
		...
		if(Hub.updateLayers){
			Hub.updateLayers = false;
			Hub.currentView.setupLayers();
		}
		if(Hub.updateView){
    		Hub.updateView = false;
    		for(int i=0;i<Hub.viewLayer.size();++i){
    			if(!Hub.viewLayer.get(i).isUpToDate()){
    				Hub.viewLayer.get(i).updateView();
    			}
    		}
        }
		GL11.glTranslatef(-0.7519f+viewX, -0.564f+viewY, -1.107f+viewZ);
		GL11.glScalef(1.5f, 1.15f, 1f);
