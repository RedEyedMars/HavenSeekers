...
public class GLApp {
    ...
    public void handleEvents() {
        ...
        if (mouseDX != 0 || mouseDY != 0 || mouseDW != 0) {
            ...
            //Log.log("DX=" + mouseDX + " DY=" + mouseDY + " cursorX=" + cursorX);
        }
        ...
        while ( Keyboard.next() )  {
            ...
            if (Keyboard.getEventKeyState()) {    // key was just pressed, trigger keyDown()
                ...
                keyDown(Keyboard.getEventCharacter(),Keyboard.getEventKey());
            }
            else {
    ...
    public void loadSettings(String configFilename)
    {
        ...
        if (configFileIn == null) {
            Log.log("GLApp.loadSettings() warning: config file " + configFilename + " not found, will use default settings.");
            return;
        }
        else {
        	try { settings.load(configFileIn); }
        	catch (Exception e) {
        		Log.log("GLApp.loadSettings() warning: " + e);
        		return;
        	}
        }
    ...
    public boolean initDisplay() {
        ...
        Log.log("GLApp.initDisplay(): Current display mode is " + origDM);
        ...
        try {
            ...
            Log.log("GLApp.initDisplay(): Setting display mode to " + DM + " with pixel depth = " + depthBufferBits);
        ...
        try {
            ...
            //Log.log("GLApp.initDisplay(): Created OpenGL window.");
        }
    ...
    public static Properties loadPropertiesFile(String configFilename)
    {
        ...
        try { settings.load(getInputStream(configFilename)); }
            ...
            Log.log("GLApp.loadPropertiesFile() warning: " + e);
            return null;
        }
        return props;
    }
    ...
    public static int makeTexture(InputStream textureImagePath)
    {
		int textureHandle = 0;
		GLImage textureImg = loadImage(textureImagePath);
		if (textureImg != null) {
     ...
     */
    public static int makeTexture(InputStream textureImagePath, boolean mipmap, boolean anisotropic)
    {
        int textureHandle = 0;
        GLImage textureImg = loadImage(textureImagePath);
        if (textureImg != null) {
    ...
    public static int makeTexture(GLImage textureImg)
    {
        if ( textureImg != null ) {
            if (isPowerOf2(textureImg.w) && isPowerOf2(textureImg.h)) {
                ...
                Log.log("GLApp.makeTexture(GLImage) Warning: not a power of two: " + textureImg.w + "," + textureImg.h);
    ...
    public static int makeTextureForScreen(int screenSize)
    {
        ...
        Log.log("GLApp.makeTextureForScreen(): made texture for screen with size " + screenTextureSize);
        // get a new empty texture
        int textureHandle = allocateTexture();
	...
	*/
    ...
    public static GLImage loadImage(String imgFilename) {
        GLImage img = new GLImage(imgFilename,false,false);
        if (img.isLoaded()) {
            return img;
        }
        return null;
    ...
    public static GLImage loadImage(InputStream imgFilename) {
        GLImage img = new GLImage(imgFilename,false,false);
        if (img.isLoaded()) {
            return img;
        }
     ...
     * @return
     */
    public static ByteBuffer loadImagePixels(InputStream imgFilename) {
     ...
     * @see       destroyFont()
     */
    public static boolean buildFont(InputStream charSetImage, int fontWidth)
    {
    	...
    	if (pb != null) {
    		try {
    			...
    			if (pb.isBufferLost()) {
    				int w = pb.getWidth();
    				int h = pb.getHeight();
    				Log.log("GLApp.selectPbuffer(): Buffer contents lost - recreating the pbuffer");
    				pb.destroy();
    				pb = makePbuffer(w, h);
    			}
	...
	public static InputStream getInputStream(String filename) {
    	...
    	try {
    		...
    		Log.log("GLApp.getInputStream (" + filename + "): " + ioe);
    		if (in != null) {
    			try {
    				in.close();
    			}
    		...
    		Log.log("GLApp.getInputStream (" + filename + "): " + e);
    	}
    	...
    	if (in == null && rootClass != null) {
    		...
    		catch (Exception ue) {Log.log("GLApp.getInputStream(): Can't find resource: " + ue);}
    		//Log.log("GLApp.getInputStream (" +filename+ "): try jar resource url=" + u);
    		if (u != null) {
    			try {
    				in = u.openStream();
    			}
    			catch (Exception e) {
    				Log.log("GLApp.getInputStream (" +filename+ "): Can't load from jar: " + e);
    			}
    		}
    		...
    		if (in == null && appletBaseURL != null) {
    			...
    			catch (Exception ue) {Log.log("GLApp.getInputStream(): Can't make applet base url: " + ue);}
    			//Log.log("GLApp.getInputStream (" +filename+ "): try applet base url=" + u);
    			try {
    				in = u.openStream();
    			}
    			catch (Exception e) {
    				Log.log("GLApp.getInputStream (" +filename+ "): Can't load from applet base URL: " + e);
    			}
    		}
    	}
    	return in;
    ...
    public static byte[] getBytesFromStream(InputStream is) {
    	...
    	Log.log("getBytesFromStream() read " + numCopied + " bytes.");
